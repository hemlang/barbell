...
usage() {
    ...
    echo ""
    echo "Benchmarks: fib, array_sum, string_concat, primes_sieve, quicksort, binary_tree, graph_bfs, json_serialize, json_deserialize, unicode_normalize"
...
run_all() {
    ...
    if [[ -n "$BENCHMARK" ]]; then
        ...
        benchmarks="fib array_sum string_concat primes_sieve quicksort binary_tree graph_bfs json_serialize json_deserialize unicode_normalize"
    fi
